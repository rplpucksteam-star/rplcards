...
# ==================== КОНСТАНТЫ БОЕВОГО ПРОПУСКА ====================
BATTLE_PASS_LEVELS = 250
BATTLE_PASS_XP_PER_LEVEL = 1000
BATTLE_PASS_TASKS_COUNT = 6
BATTLE_PASS_TASK_REFRESH_HOURS = 12

BATTLE_PASS_TASKS = [
    {"id": "goals_10", "name": "Забить 10 голов", "check": "goals_count", "need": 10, "xp": 750},
    {"id": "win_0", "name": "Выиграть на 0", "check": "win_zero", "need": 1, "xp": 750},
    {"id": "wheel", "name": "Прокрутить колесо удачи", "check": "wheel", "need": 1, "xp": 750},
    {"id": "free_card", "name": "Открыть бесплатную карту", "check": "free_card", "need": 1, "xp": 750},
    {"id": "buy_pack", "name": "Купить любой пак", "check": "buy_pack", "need": 1, "xp": 750},
    {"id": "coin", "name": "Сыграть в монетку", "check": "coin", "need": 1, "xp": 750},
    {"id": "rps", "name": "Сыграть в КНБ", "check": "rps", "need": 1, "xp": 750},
    {"id": "slots", "name": "Сыграть в слоты", "check": "slots", "need": 1, "xp": 750},
    {"id": "dice", "name": "Сыграть в кости", "check": "dice", "need": 1, "xp": 750},
    {"id": "daily", "name": "Забрать ежедневную награду", "check": "daily", "need": 1, "xp": 750},
    {"id": "epic_from_daily", "name": "Выбить эпическую карточку из ежедневной", "check": "epic_daily", "need": 1, "xp": 750},
    {"id": "market_buy", "name": "Купить что-то на торговой площадке", "check": "market_buy", "need": 1, "xp": 750},
    {"id": "buy_booster", "name": "Купить бустер", "check": "buy_booster", "need": 1, "xp": 750},
    {"id": "market_sell", "name": "Продать что-то на торговой площадке", "check": "market_sell", "need": 1, "xp": 750},
    {"id": "rob_bank", "name": "Ограбить банк успешно", "check": "rob_bank", "need": 1, "xp": 750},
    {"id": "coach", "name": "Провести успешную тренировку", "check": "coach", "need": 1, "xp": 750},
    {"id": "match_5", "name": "Сыграть 5 игр в подборе", "check": "match_count", "need": 5, "xp": 750},
    {"id": "match_15", "name": "Сыграть 15 игр в подборе", "check": "match_count", "need": 15, "xp": 750},
    {"id": "match_50", "name": "Сыграть 50 игр в подборе", "check": "match_count", "need": 50, "xp": 2000},
    {"id": "edit_roster", "name": "Изменить состав", "check": "edit_roster", "need": 1, "xp": 750},
    {"id": "win_match", "name": "Выиграть матч в подборе", "check": "win_match", "need": 1, "xp": 750},
    {"id": "craft_legend", "name": "Скрафтить легендарную карточку", "check": "craft_legend", "need": 1, "xp": 750},
    {"id": "sell_card_system", "name": "Продать карточку системе", "check": "sell_card_system", "need": 1, "xp": 750},
    {"id": "market_list", "name": "Выставить карточку на рынок", "check": "market_list", "need": 1, "xp": 750},
    {"id": "get_card_from_wheel", "name": "Получить карту из колеса удачи", "check": "wheel_card", "need": 1, "xp": 750},
]
...
def init_db():
    conn = get_db()
    c = conn.cursor()
    # Все таблицы, включая battle_passes, battle_pass_rewards, user_battle_pass, user_battle_pass_tasks
    c.execute('''
    ...
    ''')
    # Остальные таблицы (source_channels, target_chats, support_messages, admins, bot_config, collections, card_teams, cards, user_cards, user_rosters, packs, pack_cards, user_pack_buys, freepack_config, promo_codes, user_promocodes, market, user_boosters, patches)
    c.execute('''
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS battle_pass_rewards (
            id SERIAL PRIMARY KEY,
            pass_id INTEGER REFERENCES battle_passes(id) ON DELETE CASCADE,
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS user_battle_pass_tasks (
            ...
            PRIMARY KEY(user_id, pass_id, task_id)
        )
    ''')
    conn.commit()
    conn.close()
...
# ==================== ФУНКЦИИ БОЕВОГО ПРОПУСКА ====================
def create_battle_pass(title, description, end_date, photo_id=None):
    conn = get_db()
    c = conn.cursor()
    c.execute("""
        INSERT INTO battle_passes (title, description, end_date, photo_id)
        VALUES (%s, %s, %s, %s) RETURNING id
    """, (title, description, end_date, photo_id))
    pass_id = c.fetchone()['id']
    conn.commit()
    conn.close()
    return pass_id

def generate_rewards_for_pass(pass_id):
    conn = get_db()
    c = conn.cursor()
    c.execute("DELETE FROM battle_pass_rewards WHERE pass_id = %s", (pass_id,))
    reward_pool = [
        {"type": "money", "params": [100, 200], "weight": 30},
        {"type": "money", "params": [1, 10000], "weight": 40},
        {"type": "money", "params": [5000, 15000], "weight": 25},
        {"type": "money", "params": [25000, 50000], "weight": 12},
        {"type": "money", "params": [0, 500000], "weight": 3},
        {"type": "money", "params": [100000, 250000], "weight": 2},
        {"type": "card", "params": {"rarity": "Редкая", "min_ovr": 50, "max_ovr": 60}, "weight": 20},
        {"type": "card", "params": {"rarity": "Редкая"}, "weight": 18},
        {"type": "card", "params": {"rarity": "Очень редкая"}, "weight": 15},
        {"type": "card", "params": {"rarity": "Эпическая"}, "weight": 10},
        {"type": "card", "params": {"rarity": "Мифическая"}, "weight": 5},
        {"type": "card", "params": {"rarity": "Легендарная"}, "weight": 2},
        {"type": "card", "params": {"position": "Goalie", "max_ovr": 91}, "weight": 4},
        {"type": "booster", "params": {"booster_type": "rare"}, "weight": 15},
        {"type": "booster", "params": {"booster_type": "epic"}, "weight": 10},
        {"type": "booster", "params": {"booster_type": "mythic"}, "weight": 4},
        {"type": "booster", "params": {"booster_type": "legendary", "limit": 10}, "weight": 2},
        {"type": "cards_set", "params": {"count": 3, "min_ovr": 50, "max_ovr": 60}, "weight": 12},
        {"type": "cards_set", "params": {"count": 3, "any": True}, "weight": 6},
        {"type": "cards_set", "params": {"count": 3, "max_ovr": 89}, "weight": 7},
        {"type": "reset_cd", "params": {"cd": "wheel"}, "weight": 8},
        {"type": "reset_cd", "params": {"cd": "free_card"}, "weight": 8},
        {"type": "reset_cd", "params": {"cd": "job_bank"}, "weight": 5},
        {"type": "reset_cd", "params": {"cd": "job_coach"}, "weight": 5},
        {"type": "discount", "params": {"percent": 10}, "weight": 6},
        {"type": "nothing", "weight": 10},
    ]
    legendary_booster_count = 0
    big_money_count = 0
    money_100k_used = False
    for level in range(1, BATTLE_PASS_LEVELS + 1):
        available = [r for r in reward_pool]
        if legendary_booster_count >= 10:
            available = [r for r in available if not (r["type"] == "booster" and r["params"].get("booster_type") == "legendary")]
        if big_money_count >= 2:
            available = [r for r in available if not (r["type"] == "money" and r["params"] == [0, 500000])]
        if money_100k_used or level < 50:
            available = [r for r in available if not (r["type"] == "money" and r["params"] == [100000, 250000])]
        weights = [r["weight"] for r in available]
        chosen = random.choices(available, weights=weights, k=1)[0]
        if chosen["type"] == "booster" and chosen["params"].get("booster_type") == "legendary":
            legendary_booster_count += 1
        if chosen["type"] == "money" and chosen["params"] == [0, 500000]:
            big_money_count += 1
        if chosen["type"] == "money" and chosen["params"] == [100000, 250000]:
            money_100k_used = True
        c.execute("""
            INSERT INTO battle_pass_rewards (pass_id, level, reward_type, reward_data)
            VALUES (%s, %s, %s, %s)
        """, (pass_id, level, chosen["type"], json.dumps(chosen["params"])))
    conn.commit()
    conn.close()

def get_active_battle_pass():
    conn = get_db()
    c = conn.cursor()
    now = datetime.now()
    c.execute("""
...
def get_user_battle_pass(user_id, pass_id):
    conn = get_db()
    c = conn.cursor()
    c.execute("""
        SELECT * FROM user_battle_pass
        WHERE user_id = %s AND pass_id = %s
    """, (user_id, pass_id))
    row = c.fetchone()
    conn.close()
    return row

def create_user_battle_pass(user_id, pass_id):
    conn = get_db()
    c = conn.cursor()
    c.execute("""
        INSERT INTO user_battle_pass (user_id, pass_id, level, xp, last_task_refresh)
        VALUES (%s, %s, 0, 0, %s)
        ON CONFLICT (user_id, pass_id) DO NOTHING
    """, (user_id, pass_id, datetime.now()))
    conn.commit()
    conn.close()

def refresh_user_tasks(user_id, pass_id):
    conn = get_db()
    c = conn.cursor()
    c.execute("DELETE FROM user_battle_pass_tasks WHERE user_id = %s AND pass_id = %s", (user_id, pass_id))
    task_ids = [t["id"] for t in BATTLE_PASS_TASKS]
    selected = random.sample(task_ids, min(BATTLE_PASS_TASKS_COUNT, len(task_ids)))
    for task_id in selected:
        c.execute("""
            INSERT INTO user_battle_pass_tasks (user_id, pass_id, task_id, progress, completed)
            VALUES (%s, %s, %s, 0, FALSE)
        """, (user_id, pass_id, task_id))
    c.execute("""
        UPDATE user_battle_pass
        SET last_task_refresh = %s
        WHERE user_id = %s AND pass_id = %s
    """, (datetime.now(), user_id, pass_id))
    conn.commit()
    conn.close()

def get_user_tasks(user_id, pass_id):
    conn = get_db()
    c = conn.cursor()
    # Строим VALUES для JOIN
    task_values = [(t["id"], t["name"], t["xp"]) for t in BATTLE_PASS_TASKS]
    query = """
        SELECT t.*, task_def.name, task_def.xp
        FROM user_battle_pass_tasks t
        JOIN (VALUES %s) AS task_def(id, name, xp) ON t.task_id = task_def.id
        WHERE t.user_id = %s AND t.pass_id = %s
    """
    # Формируем строку VALUES
    values_str = ','.join(['(%s,%s,%s)'] * len(task_values))
    flat = [val for sub in task_values for val in sub] + [user_id, pass_id]
    c.execute(query.replace('%s', values_str) if '%s' in query else query, flat)
    rows = c.fetchall()
    conn.close()
    return rows

def add_battle_pass_xp(user_id, pass_id, amount):
    if amount <= 0:
        return
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT level, xp FROM user_battle_pass WHERE user_id = %s AND pass_id = %s FOR UPDATE", (user_id, pass_id))
    row = c.fetchone()
    if not row:
        conn.close()
        return
    level = row["level"]
    xp = row["xp"] + amount
    while xp >= BATTLE_PASS_XP_PER_LEVEL and level < BATTLE_PASS_LEVELS:
        xp -= BATTLE_PASS_XP_PER_LEVEL
        level += 1
        give_battle_pass_reward(c, user_id, pass_id, level)
    c.execute("""
        UPDATE user_battle_pass
        SET level = %s, xp = %s
        WHERE user_id = %s AND pass_id = %s
    """, (level, xp, user_id, pass_id))
    conn.commit()
    conn.close()

def give_battle_pass_reward(cursor, user_id, pass_id, level):
    cursor.execute("""
        SELECT reward_type, reward_data
        FROM battle_pass_rewards
        WHERE pass_id = %s AND level = %s
    """, (pass_id, level))
    ...
    if rtype == "money":
        amount = random.randint(data[0], data[1])
        ...
        if "rarity" in data:
            query += " AND rarity = %s"
            params.append(data["rarity"])
        if "min_ovr" in data:
            query += " AND ovr >= %s"
            params.append(data["min_ovr"])
        if "max_ovr" in data:
            query += " AND ovr <= %s"
            params.append(data["max_ovr"])
        if "position" in data:
        ...
        if booster_type in BOOSTERS:
            b = BOOSTERS[booster_type]
            now = datetime.now()
            active_until = now + timedelta(hours=b["hours"])
            cursor.execute("""
                UPDATE users
                SET balance = balance - %s,
                    ...
                    END
                WHERE user_id = %s
            """, (b["price"], b["xp_percent"], active_until, active_until,
                  b["money_percent"], b["money_percent"], active_until, active_until,
                  user_id))
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (user_id, booster_type, b["xp"], b["xp_percent"],
                  b["money_percent"], active_until))
    elif rtype == "cards_set":
        count = data.get("count", 3)
        any_card = data.get("any", False)
        min_ovr = data.get("min_ovr", 0)
        max_ovr = data.get("max_ovr", 99)
        query = "SELECT id FROM cards WHERE ovr BETWEEN %s AND %s"
        params = [min_ovr, max_ovr]
        if not any_card:
            query += " AND rarity NOT IN ('Секретная')"
        cursor.execute(query, params)
        cards = cursor.fetchall()
        if cards:
        ...
        cd = data["cd"]
        if cd == "wheel":
            cursor.execute("UPDATE users SET last_wheel_spin = %s WHERE user_id = %s", (datetime.min, user_id))
        elif cd == "free_card":
            cursor.execute("UPDATE users SET free_card_cooldown_reset_until = %s WHERE user_id = %s", (datetime.max, user_id))
        elif cd == "job_bank":
            cursor.execute("UPDATE users SET illegal_cooldown_until = %s WHERE user_id = %s", (datetime.min, user_id))
        elif cd == "job_coach":
            cursor.execute("UPDATE users SET coach_cooldown_until = %s WHERE user_id = %s", (datetime.min, user_id))
    elif rtype == "discount":
        percent = data.get("percent", 10)
        cursor.execute("UPDATE users SET discount_percent = %s WHERE user_id = %s", (percent, user_id))
    # nothing – ничего не делаем

def check_and_complete_bp_task(user_id, task_check_type, value=1):
    pass_info = get_active_battle_pass()
    if not pass_info:
        return
    pass_id = pass_info["id"]
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT 1 FROM user_battle_pass WHERE user_id = %s AND pass_id = %s", (user_id, pass_id))
    if not c.fetchone():
        conn.close()
        return
    task_def = None
    for t in BATTLE_PASS_TASKS:
        if t["check"] == task_check_type:
            task_def = t
            break
    if not task_def:
        conn.close()
    ...
    c.execute("""
        ...
        FROM user_battle_pass_tasks
        WHERE user_id = %s AND pass_id = %s AND task_id = %s
    """, (user_id, pass_id, task_def["id"]))
    task = c.fetchone()
    if not task or task["completed"]:
        conn.close()
        return
    new_progress = min(task["progress"] + value, task_def["need"])
    completed = new_progress >= task_def["need"]
    c.execute("""
        UPDATE user_battle_pass_tasks
        SET progress = %s, completed = %s
        WHERE user_id = %s AND pass_id = %s AND task_id = %s
    """, (new_progress, completed, user_id, pass_id, task_def["id"]))
    if completed:
        add_battle_pass_xp(user_id, pass_id, task_def["xp"])
    conn.commit()
    conn.close()
...
def admin_menu_keyboard():
    return ReplyKeyboardMarkup([
        ...
        ["👥 Список игроков", "➕ Добавить патч"],
        ["➕ Добавить боевой пропуск", "🚪 Выйти"]
    ], resize_keyboard=True)
...
def welcome_inline_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("🎖 Боевой пропуск", callback_data="battle_pass_menu")],
...
async def rplcards_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not card:
        ...
        conn.commit()
        conn.close()
        await update.message.reply_text("🎉 **Невероятно!** У вас уже есть все доступные карточки, поэтому вместо дубликата вам начислено **3 000 RPLCoin**!", parse_mode="Markdown")
        return
    card_id = card['id']
    c.execute('''
    ...
    conn.commit()
    conn.close()
    # Проверка задания "открыть бесплатную карту"
    check_and_complete_bp_task(user.id, "free_card", 1)
...
async def inventory_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "refresh_inv":
        ...
        conn.commit()
        conn.close()
        check_and_complete_bp_task(user.id, "sell_card_system", 1)
        ...
        conn.commit()
        conn.close()
        check_and_complete_bp_task(user.id, "craft_legend", 1)
...
async def market_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "refresh_market":
        ...
        conn.commit()
        conn.close()
        check_and_complete_bp_task(user.id, "market_buy", 1)
        try:
...
async def execute_market_list_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        conn.commit()
        conn.close()
        check_and_complete_bp_task(user.id, "market_list", 1)
...
# ---- МАГАЗИН (ПАКИ И БУСТЕРЫ) ----
async def shop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_pm_registered(update, context):
        return
    await update.message.reply_text("🛍 **МАГАЗИН**\n\nВыберите нужный раздел:", reply_markup=store_keyboard(), parse_mode="Markdown")
...
async def shop_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("preview_pack_"):
        ...
        conn.commit()
        conn.close()
        check_and_complete_bp_task(user.id, "buy_pack", 1)
...
async def store_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "store_packs":
        await show_shop(update, context)
    elif query.data == "store_boosters":
...
async def booster_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    check_and_complete_bp_task(user_id, "buy_booster", 1)
...
async def wheel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if prize == "reset_cd":
    ...
    elif prize in ("card_50_65", "card_70_80", "card_80_85", "rare", "very_rare", "epic", "mythic"):
        if prize == "card_50_65":
        ...
        if card:
            ...
            prize_text = f"🃏 **Карточка:** {card['nickname']} ({card['ovr']} OVR)"
            card_given = True
        else:
            fallback = 20000 if "50_65" in prize else 40000 if "70_80" in prize else 70000 if "80_85" in prize else 10000 if prize == "rare" else 20000 if prize == "very_rare" else 40000 if prize == "epic" else 80000
            c.execute("UPDATE users SET balance = balance + %s WHERE user_id = %s", (fallback, user.id))
            prize_text = f"💵 Компенсация {fallback} RPLCoin"
    elif prize == "discount":
        disc = random.randint(10, 30)
        c.execute("UPDATE users SET discount_percent = %s WHERE user_id = %s", (disc, user.id))
        prize_text = f"🏷 **Скидка {disc}%** на магазин и торговую площадку!"
    elif prize == "custom_card":
        set_config("custom_card_prize_claimed", "1")
        prize_text = "🎨 **Уникальный приз: создание своей карты!** Свяжитесь с администратором."
    else:
        prize_text = "💨 **Ничего!** Повезет в следующий раз."
    conn.commit()
    conn.close()
    check_and_complete_bp_task(user.id, "wheel", 1)
    if card_given:
        check_and_complete_bp_task(user.id, "wheel_card", 1)
    await update.message.reply_text(f"🎡 **Колесо удачи прокручено!**\n\n{prize_text}", parse_mode="Markdown")
...
async def daily_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if streak == 1:
        ...
        reward_text = "Скидку 15% в магазине и торговой площадке 🏷"
    elif streak == 5:
    ...
    conn.commit()
    conn.close()
    check_and_complete_bp_task(user.id, "daily", 1)
    if epic_daily:
        check_and_complete_bp_task(user.id, "epic_daily", 1)
    await update.message.reply_text(
...
async def job_coach_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        ...
        check_and_complete_bp_task(user.id, "coach", 1)
    else:
        c.execute("UPDATE users SET coach_cooldown_until = %s WHERE user_id = %s", (new_cd, user.id))
        conn.commit()
        conn.close()
...
async def job_illegal_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    if success and action == "job_ill_bank":
        check_and_complete_bp_task(user.id, "rob_bank", 1)
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад к работам", callback_data="jobs_menu")]])
    await query.edit_message_text(f"🕵️‍♂️ **Результат работы «Нелегал»:**\n\n{res_text}", reply_markup=kb, parse_mode="Markdown")

# ---- МИНИ-ИГРЫ (с проверками заданий) ----
...
# В целях экономии места, я не буду дублировать все мини-игры полностью, но они уже есть в исходном коде.
# Я добавлю только вызовы check_and_complete_bp_task в их обработчиках.

# (Здесь должны быть функции coin_command, coin_receive_bet, coin_callback_handler, rps_command, rps_receive_bet, rps_callback_handler, slots_command, slots_receive_bet, dice_command, dice_receive_bet)
# В этих функциях после завершения игры и обновления баланса нужно добавить:
# check_and_complete_bp_task(user.id, "coin", 1)  (для монетки)
# check_and_complete_bp_task(user.id, "rps", 1)   (для КНБ)
# check_and_complete_bp_task(user.id, "slots", 1) (для слотов)
# check_and_complete_bp_task(user.id, "dice", 1)  (для костей)

# Я не буду вставлять полный код этих функций, так как они огромные, но все они есть в исходнике.
# Просто добавьте эти вызовы в соответствующие места.
...
# ---- БОЕВОЙ ПРОПУСК - ОСНОВНЫЕ ОБРАБОТЧИКИ ----
async def battle_pass_menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_pm_registered(update, context):
        return
    user = update.effective_user
    pass_info = get_active_battle_pass()
    if not pass_info:
        await update.message.reply_text("❌ Активный боевой пропуск отсутствует.")
        return
    pass_id = pass_info["id"]
    create_user_battle_pass(user.id, pass_id)
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT last_task_refresh FROM user_battle_pass WHERE user_id = %s AND pass_id = %s", (user.id, pass_id))
    row = c.fetchone()
    if row and row["last_task_refresh"]:
        last_refresh = row["last_task_refresh"]
        if isinstance(last_refresh, str):
            last_refresh = datetime.fromisoformat(last_refresh)
        if datetime.now() - last_refresh > timedelta(hours=BATTLE_PASS_TASK_REFRESH_HOURS):
            refresh_user_tasks(user.id, pass_id)
    else:
        refresh_user_tasks(user.id, pass_id)
    conn.close()
    user_progress = get_user_battle_pass(user.id, pass_id)
    tasks = get_user_tasks(user.id, pass_id)
    text = f"🎖 **{pass_info['title']}**\n"
    text += f"📅 До окончания: {pass_info['end_date'].strftime('%d.%m.%Y %H:%M')}\n"
    text += f"📝 {pass_info['description']}\n\n"
    text += f"📊 Ваш прогресс: Уровень **{user_progress['level']}** (XP: {user_progress['xp']}/{BATTLE_PASS_XP_PER_LEVEL})\n"
    text += f"🏅 Следующая награда: уровень {user_progress['level']+1}\n\n"
    text += "📋 **Активные задания:**\n"
    for t in tasks:
        status = "✅" if t["completed"] else f"({t['progress']}/? )"
        text += f"{status} {t['name']} (+{t['xp']} XP)\n"
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎁 Все награды", callback_data=f"bp_rewards_{pass_id}")],
        [InlineKeyboardButton("🔄 Обновить", callback_data="bp_refresh")]
    ])
    if pass_info['photo_id']:
        try:
            await update.message.reply_photo(photo=pass_info['photo_id'], caption=text, reply_markup=kb, parse_mode="Markdown")
            return
        except Exception:
            pass
    await update.message.reply_text(text, reply_markup=kb, parse_mode="Markdown")

async def bp_rewards_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    pass_id = int(query.data.split("_")[2])
    conn = get_db()
    c = conn.cursor()
    c.execute("""
        SELECT level, reward_type, reward_data
        FROM battle_pass_rewards
        WHERE pass_id = %s
        ORDER BY level
    """, (pass_id,))
    ...
    conn.close()
    if not rewards:
        await query.edit_message_text("❌ Награды ещё не сгенерированы.")
        return
    ...
    for r in rewards:
        ...
        rtype = r["reward_type"]
        if rtype == "money":
            text += f"Ур. {level}: 💰 {r['reward_data'][0]}-{r['reward_data'][1]} RPL\n"
        elif rtype == "card":
            text += f"Ур. {level}: 🃏 Карта (условия: {r['reward_data']})\n"
        elif rtype == "booster":
            text += f"Ур. {level}: 🚀 Бустер {r['reward_data']['booster_type']}\n"
        elif rtype == "cards_set":
            text += f"Ур. {level}: 📦 Набор карт ({r['reward_data']})\n"
        elif rtype == "reset_cd":
            text += f"Ур. {level}: 🔄 Обнуление КД ({r['reward_data']['cd']})\n"
        elif rtype == "discount":
            text += f"Ур. {level}: 🏷 Скидка {r['reward_data']['percent']}%\n"
        elif rtype == "nothing":
            text += f"Ур. {level}: 🎁 Секретный приз\n"
        else:
            text += f"Ур. {level}: {rtype} ({r['reward_data']})\n"
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="bp_back")]])
    await query.edit_message_text(text, reply_markup=kb, parse_mode="Markdown")

async def bp_back_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await battle_pass_menu_command(update, context)

async def bp_refresh_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await battle_pass_menu_command(update, context)
...
async def start_game_pvp(p1_id, p2_id, p1_chat_id, p2_chat_id, p1_msg_id, p2_msg_id, context):
    ...
    try:
        ...
        for period in range(1, 4):
            ...
            for tick in range(1, 4):
                ...
                if rand_val < prob_p1:
                    ...
                    # Проверка задания "забить 10 голов"
                    check_and_complete_bp_task(p1_id, "goals_count", 1)
                    ...
                    all_events.append(evt)
                    check_and_complete_bp_task(p2_id, "goals_count", 1)
                else:
        ...
        if score1 > score2:
            ...
            check_and_complete_bp_task(p1_id, "win_match", 1)
            check_and_complete_bp_task(p1_id, "win_zero", 1 if score2 == 0 else 0)  # проверка на 0
            check_and_complete_bp_task(p2_id, "win_zero", 0)  # проигравший не выиграл на 0
        elif score2 > score1:
            res_text = f"🎉 **ПОБЕДА 🔵 {name2}!** Счет: **{score1} - {score2}**"
            apply_match_stats(c, p2_id, win=True)
            apply_match_stats(c, p1_id, win=False)
            check_and_complete_bp_task(p2_id, "win_match", 1)
            check_and_complete_bp_task(p2_id, "win_zero", 1 if score1 == 0 else 0)
            check_and_complete_bp_task(p1_id, "win_zero", 0)
        else:
            res_text = f"🤝 **НИЧЬЯ!** Счет: **{score1} - {score2}**"
            apply_match_stats(c, p1_id, win=None)
            apply_match_stats(c, p2_id, win=None)
            check_and_complete_bp_task(p1_id, "win_zero", 0)
            check_and_complete_bp_task(p2_id, "win_zero", 0)
        # Проверка количества сыгранных матчей (для обоих)
        check_and_complete_bp_task(p1_id, "match_count", 1)
        check_and_complete_bp_task(p2_id, "match_count", 1)
        conn.commit()
        conn.close()
        final_text = (
            f"🏁 **МАТЧ ЗАВЕРШЕН!**\n\n{res_text}\n\n"
...
async def start_game_vs_ai(p1_id, chat_id, msg_id, context):
    ...
    try:
        ...
        for period in range(1, 4):
            ...
            for tick in range(1, 4):
                ...
                if rand_val < prob_p1:
                    ...
                    if evt_type == "save1":
                        all_events.append(f"🧤 **{minute}' СЕЙВ!** {p1_cards['goalie']['nickname']} выручает команду.")
                    elif evt_type == "save2":
                        all_events.append(f"🧤 **{minute}' СЕЙВ!** ИИ Вратарь забирает шайбу в ловушку.")
                    elif evt_type == "post":
                        all_events.append(f"🏒 **{minute}' ШТАНГА!** Опаснейший бросок сотрясает каркас ворот!")
                    elif evt_type == "hit":
                        all_events.append(f"💥 **{minute}' СИЛОВОЙ ПРИЕМ!** Жесткая борьба у бортов.")
                    elif evt_type == "var_cancel":
                        all_events.append(f"⚖️ **{minute}' VAR:** Гол отменен из-за вне игры!")
                    else:
                        all_events.append(f"🥊 **{minute}' ДРАКА!** Судья разнял хоккеистов.")
                recent_events = "\n".join(all_events[-6:])
        ...
        if score1 > score2:
            ...
            apply_match_stats(c, p1_id, win=True)
            check_and_complete_bp_task(p1_id, "win_match", 1)
            check_and_complete_bp_task(p1_id, "win_zero", 1 if score2 == 0 else 0)
        elif score2 > score1:
...
async def coin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    check_and_complete_bp_task(user.id, "coin", 1)
    names = {"eagle": "🦅 Орёл", "tail": "🪙 Решка"}
    text = f"🪙 **Результат Орёл и Решка:**\n\n👤 Ваш выбор: {names[player_choice]}\n🪙 Результат подброса: {names[bot_choice]}\n\n{res_str}"
...
async def rps_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    check_and_complete_bp_task(user.id, "rps", 1)
    text = f"🎮 **Результат игры КНБ:**\n\n👤 Ваш выбор: {emojis[player_choice]}\n🤖 Выбор бота: {emojis[bot_choice]}\n\n{res_str}"
    await query.message.edit_text(text, parse_mode="Markdown")
...
async def slots_receive_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        check_and_complete_bp_task(user.id, "slots", 1)
...
async def dice_receive_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        check_and_complete_bp_task(user.id, "dice", 1)
...
async def inline_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "battle_pass_menu":
        await battle_pass_menu_command(update, context)
    elif data.startswith("bp_rewards_"):
        await bp_rewards_callback(update, context)
    elif data == "bp_back":
        await bp_back_callback(update, context)
    elif data == "bp_refresh":
        await bp_refresh_callback(update, context)
    return
...
# ==================== ПРОВЕРКА ЛС (общая) ====================
# Функция check_pm_registered уже была в части 2, но на всякий случай продублирую
# (она уже есть, так что можно не добавлять)

# ==================== ФУНКЦИЯ MAIN (РЕГИСТРАЦИЯ ВСЕХ ОБРАБОТЧИКОВ) ====================
async def post_init_hook(application: Application):
    # Запускаем фоновую задачу уведомлений о кулдаунах (из исходного кода)
    asyncio.create_task(cooldown_notifier_loop(application))

async def cooldown_notifier_loop(application: Application):
    # Эта функция уже была в исходном коде, поэтому я её не дублирую, но для полноты она должна быть.
    # Я предполагаю, что она уже есть. Если нет – возьмите из исходника.
    # Я приведу её кратко, но она уже есть в части 1? Нет, в части 1 её не было, но она была в исходном коде пользователя.
    # Я добавлю её здесь, чтобы не было ошибок.
...
def main():
    ...
    app.add_handler(CommandHandler("battlepass", battle_pass_menu_command))
    ...
    app.add_handler(MessageHandler(filters.Regex("^🎖 Боевой пропуск$"), battle_pass_menu_command))
    ...
    # ---- АДМИН-МЕНЮ (основные кнопки) ----
    app.add_handler(MessageHandler(filters.Regex("^➕ Добавить каналы$") & filters.ChatType.PRIVATE, admin_buttons))
    app.add_handler(MessageHandler(filters.Regex("^➕ Добавить чаты$") & filters.ChatType.PRIVATE, admin_buttons))
    ...
    app.add_handler(MessageHandler(filters.Regex("^📦 Выставить пак в магазин$") & filters.ChatType.PRIVATE, admin_buttons))
    app.add_handler(MessageHandler(filters.Regex("^🔍 Инвентарь игрока$") & filters.ChatType.PRIVATE, admin_buttons))
    app.add_handler(MessageHandler(filters.Regex("^👥 Список игроков$") & filters.ChatType.PRIVATE, admin_buttons))
    app.add_handler(MessageHandler(filters.Regex("^➕ Добавить патч$") & filters.ChatType.PRIVATE, admin_buttons))
    app.add_handler(MessageHandler(filters.Regex("^➕ Добавить боевой пропуск$") & filters.ChatType.PRIVATE, admin_buttons))
    ...
    # ---- АДМИН: ПРОСМОТР ИНВЕНТАРЯ ИГРОКА ----
    ...
    conv_coin = ConversationHandler(
        ...
        states={WAITING_COIN_BET: [
            MessageHandler(filters.Regex("^(" + "|".join(re.escape(k) for k in MAIN_MENU_TEXT_HANDLERS) + ")$"), bet_state_menu_escape),
    ...
    # ---- АДМИН: ВЫСТАВЛЕНИЕ ПАКА В МАГАЗИН ----
    conv_admin_shop_pack = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^📦 Выставить пак в магазин$") & filters.ChatType.PRIVATE, admin_buttons)],
        states={
    ...
    # ---- АДМИН: ДОБАВЛЕНИЕ БОЕВОГО ПРОПУСКА ----
    conv_bp = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^➕ Добавить боевой пропуск$") & filters.ChatType.PRIVATE, admin_buttons)],
        states={
            WAITING_BP_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_bp_title)],
            WAITING_BP_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_bp_desc)],
            WAITING_BP_END_DATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_bp_end_date)],
            WAITING_BP_PHOTO: [
                MessageHandler(filters.PHOTO, admin_bp_photo),
                CommandHandler("skip", admin_bp_skip),
            ],
        },
        fallbacks=[CommandHandler("cancel", lambda u,c: u.message.reply_text("Отменено."))],
        per_message=False,
    )
    app.add_handler(conv_bp)

    # ---- АДМИН: КАРТОЧКИ (ОСНОВНОЙ КОНВЕРСЕЙШН) ----
    ...
    app.add_handler(CallbackQueryHandler(inline_callback, pattern="^(back_to_main_inline|discord|website|support|duel|play_coin|play_rps|play_slots|play_dice|patches_list|patch_view_|battle_pass_menu|bp_rewards_|bp_back|bp_refresh)$"))
